...
class Database {
    ...
    async getUsers() {
        let found = await this.users.find({}).toArray()
        return found;
    }
    ...
    async createUser(person) {
        let alreadyExists = await this.users.findOne({first_name:person.first_name,last_name:person.last_name,birthday:person.birthday});
    ...
    async deleteClub(club) {
        let alreadyExists = await this.getClub(club._id);
        if (alreadyExists != null) {
    ...
    async createEvent(event) {
        let alreadyExists = await this.getEvent({_id:event._id});
        if (alreadyExists == null) {
    ...
    async deleteEvent(event) {
        let alreadyExists = await this.getEvent(event._id);
        if (alreadyExists != null) {
    ...
    async updateEvent(id, update){
        let that = this
        await this.events.findOneAndUpdate({_id: new ObjectId(id)}, update).then(function (clubs){
